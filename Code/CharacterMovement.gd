...
func _physics_process(delta):
	...
	if Input.get_action_raw_strength("Shoot") && timer >= actual_rate:
		...
		add_sibling(temp)
		temp.global_position = get_node("Gun").get_node("BulletSpawn").get("global_position")
...
func Die():
	...
	self.get_node("Player_Sprite").set("visible", false)
	self.get_node("Decal").set("visible", true)
	self.get_node("Decal").play()
